                ...
                matches.forEach(match => {
                    ...
                    if (projectKeys.includes(projectKey)) {
                        ...
                        projectKeys.forEach(key => {
                            // Create regex for matching key with optional suffix
                            const regex = new RegExp(`([^\\w]|^)(${key})([^\\w]|$)`, 'gi'); // Match project key

                            // Highlight matching substrings in the line
                            highlightedLine = highlightedLine.replace(regex, `$1<span class="substring-match">$2</span>$3`);
                        });
