                ...
                matches.forEach(match => {
                    ...
                    if (projectKeys.includes(projectKey)) {
                        ...
                        projectKeys.forEach(key => {
                            // Create regex for matching key with surrounding characters
                            const regex = new RegExp(`([^\\w]|^)(${key})(-\\d+)?`, 'gi'); // Match project key with optional JIRA ID suffix

                            // Check for matches and highlight them
                            if (regex.test(line)) {
                                highlightedLine = highlightedLine.replace(regex, `$1<span class="substring-match">$2$3</span>`);
                            }
                        });
                        exceptions.push(`Line ${index + 1}: ${highlightedLine} (Not a valid JIRA ID)`);
                    }
                });
